...
try{
	...
	if($_POST){
		...
		$finalsql = ($_POST["time"] == "month") ? "SELECT ans.年 as 年, ans.月 as 月, ans.居住地 as 居住地, " : "SELECT ans.年 as 年, ans.居住地 as 居住地, ";
		foreach($record as $k => $v){
        ...
        if($_POST["time"] == "year" && $exchangerate == "yes"){
            ...
            $finalsql = $finalsql."ans.total_people as 總人數, ans.ratio as 人數成長率 from ".$newsql." group by ans.年, ans.居住地 order by ans.年 ASC";
        else
			$finalsql = $finalsql."ans.total_people as 總人數, ans.ratio as 人數成長率 from ".$newsql." group by ans.年, ans.月, ans.居住地 order by ans.年, ans.月 ASC";
	}
