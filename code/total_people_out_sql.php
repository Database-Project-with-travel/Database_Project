...
try{
	...
	if($_POST){
        ...
        if($_POST["time"] == "month")
            $finalsql = "SELECT ans.年 as 年, ans.月 as 月, ans.國家名稱 as 國家名稱, ";
        else
            $finalsql = "SELECT ans.年 as 年, ans.國家名稱 as 國家名稱, ";
        ...
        if($_POST["time"] == "year" && $exchangerate == "yes")
            $finalsql = $finalsql.") as total_people, ans.幣別 as 幣別,
    ...
    $stmt = $conn->prepare("insert into user_history (query_sql) values (\"".$fianlsql."\");");
    $stmt->execute();
