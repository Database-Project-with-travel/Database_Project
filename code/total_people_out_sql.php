...
try{
	...
	if($_POST){
        ...
        if($_POST["time"] == "year" && $exchangerate == "yes")
            $finalsql = $finalsql.") as total_people, (case when ans.對新台幣匯率總和 = 0 then 'NULL' else ans.對新台幣匯率總和/ans.number end) 
                        from ".$sql." group by ans.年, ans.國家名稱 order by ans.年 ASC";
        else if($_POST["time"] == "month" && $exchangerate == "yes")    
            $finalsql = $finalsql.") as total_people, ans.對新台幣匯率 from ".$sql.
