...
try{
	...
	if($_POST){
        ...
        if($_POST["time"] == "month")
            $finalsql = "SELECT ans.年, ans.月, ans.國家名稱, ";
        else
            $finalsql = "SELECT ans.年, ans.國家名稱, ";
        ...
        if($_POST["time"] == "year" && $exchangerate == "yes")
            $finalsql = $finalsql.") as total_people, (case when ans.對新台幣匯率總和 = 0 then 'NULL' else ans.對新台幣匯率總和/ans.number end), ans.幣別 
                        from ".$sql." group by ans.年, ans.國家名稱 order by ans.年 ASC";
        else if($_POST["time"] == "month" && $exchangerate == "yes")    
            $finalsql = $finalsql.") as total_people, ans.對新台幣匯率, ans.幣別 from ".$sql.
                                  " group by ans.年, ans.月, ans.國家名稱 order by ans.年, ans.月 ASC";
        else if($_POST["time"] == "year" && $exchangerate == "no")
            $finalsql = $finalsql.") as total_people from ".$sql." group by ans.年, ans.國家名稱 order by ans.年 ASC";
        else
            $finalsql = $finalsql.") as total_people from ".$sql." group by ans.年, ans.月, ans.國家名稱 order by ans.年, ans.月 ASC";
		$finalsql = $finalsql.";";
    }
